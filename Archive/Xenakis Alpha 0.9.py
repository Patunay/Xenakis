...
This Version fixed the pitch mapping that previous version had. 
Now it supports a complete piano range (A0-C8) with all the quartertones in-between.
Now everything is fixed and there should not be any more problems or ambiguities regarding pitch.

Things to work on:
-Create guides for png template for pitch...(and time?)
...
-Enhance user GUI
-add support for note velocity control on empty space abvove or below, maybe even use colors
 since its fairly easy to completely hide them.
...
midicents = np.arange(2100, 10801, 50)  #Creation of midicents reference grid
midicents = midicents[:,np.newaxis]

pixel_input = np.arange(10, 185)  #Creation of pixel input reference grid
pixel_input = pixel_input[:,np.newaxis]

left = np.array([[0, 2], [0, 195]])

conversiontable = np.concatenate ((midicents,pixel_input),axis = 1) #Creation of conversion table
conversiontable[:,1] = np.flip(conversiontable[:,1])
conversiontable = np.concatenate((left,conversiontable),axis=0)
print(conversiontable)

new_pitches = [] #List where new pitches will be dumped


initial = OUTPUT[:,1]   #Initial pitch list

cntr_pchs = 0        #Counter for pitches

for sometime in end_array:      
    ind = initial[cntr_pchs]            #isolate a pitch
    location_conv = np.where(conversiontable == ind)
    lct = location_conv[0] #Get location for new pitch
    # print("location of new pitch is", lct)
    npch = conversiontable[lct,0]   #Always column number 0!
    # print("new pitch is",npch)       
    new_pitches.append(npch)         #Append to list
    cntr_pchs = cntr_pchs + 1
    # print("new ind ", cntr_pchs)

new_pitches = np.array(new_pitches)



OUTPUT = np.concatenate((new_pitches,OUTPUT),axis= 1)
OUTPUT = OUTPUT[:,[1,0,2,3]]                    #Reorder columns!



OUTPUT = np.delete(OUTPUT,2,axis=1)         #Eliminate old pitches
...
#####################################################################
    ...
    new_pitches2 = [] #List where new pitches will be dumped


    initial2 = OUTPUT2[:,1]   #Initial pitch list

    cntr_pchs = 0        #Counter for pitches

    for sometime in end_array2:      
        ind2 = initial2[cntr_pchs]            #isolate a pitch
        location_conv = np.where(conversiontable == ind2)
        lct = location_conv[0] #Get location for new pitch
        # print("location of new pitch is", lct)
        npch = conversiontable[lct,0]   #Always column number 0!
        # print("new pitch is",npch)       
        new_pitches2.append(npch)         #Append to list
        cntr_pchs = cntr_pchs + 1
        # print("new ind ", cntr_pchs)

    new_pitches2 = np.array(new_pitches2)



    OUTPUT2 = np.concatenate((new_pitches2,OUTPUT2),axis= 1)
    OUTPUT2 = OUTPUT2[:,[1,0,2,3]]                    #Reorder columns!



    OUTPUT2 = np.delete(OUTPUT2,2,axis=1)         #Eliminate old pitches
