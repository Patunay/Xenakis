...
Notes on version Alpha 1.0:

This Version fixed (for a 2nd time) the pitch mapping bug that previous version had. It was resolved with a
elegant single line of code. Yey maths!!!
(Keep https://www.wolframalpha.com/input/?i=interpolating+polynomial+calculator&assumption=%7B%22F%22%2C+%22InterpolatingPolynomialCalculator%22%2C+%22data%22%7D+-%3E%22%7B1%2C+4%2C+9%2C+16%7D%22&assumption=%7B%22FVarOpt%22%7D+-%3E+%7B%7B%22InterpolatingPolynomialCalculator%22%2C+%22data2%22%7D%7D)


Things to work on:

-Automatically prepare bach.roll (with # of voices, and rulers)
...
multiplicator = input("Enter multiplicator factor for lenght calculation, (250 if unsure) ")
...
voiceadder = (voices)  #Iterator to create voice add messages
voiceadder = np.arange(voiceadder)
...
OUTPUT[:,1] = OUTPUT[:,1]*(-50) + 11300    #Converted pitches..!!


#Handle Onsets and Durations
OUTPUT[:,0] = OUTPUT[:,0]*multiplicator
OUTPUT[:,2] = OUTPUT[:,2]*multiplicator
...
#####################################################################
"""""
Loop for Voices for N times where N = #ofVoices
"""""
#####################################################################
bachvoices = []
...
#####################################################################
    ...
    OUTPUT2[:,1] = OUTPUT2[:,1]*(-50) + 11300    #Converted pitches..!!

    #Handle Onsets and Durations
    OUTPUT2[:,0] = OUTPUT2[:,0]*multiplicator
    OUTPUT2[:,2] = OUTPUT2[:,2]*multiplicator
...
voiceaddmessage=[]
for l in voiceadder:
    voiceaddmessage.append("insert voice 1,")
voiceaddmessage = "".join(map(str,voiceaddmessage))

bach_output = voiceaddmessage + bach_output
