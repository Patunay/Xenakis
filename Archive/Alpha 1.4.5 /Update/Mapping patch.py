...
class GUI_colors:
    def __init__(self,type,weight):
        self.type = type        # Button frame, text widget,etc
        self.weight = weight

    def color():
        color = "#B4C9CF"   # Main background color
        return color
    def color_buttons():
        color = "#016F8D"
...
class conv_window:
    ...
    def __init__(self,root,title,version):
        ...
        #
        self.entry8_button = Button(self.filepath_options_frame, text="Preview Combined",bg="#016F8D",fg="white",relief=RAISED,command=self.preview_combined)
        ...
        self.frames = [self.root,self.parent_frame,self.sec_frame,self.input_frame, self.filepath_frame,self.options_frame,self.filepath_set_frame,self.filepath_list_frame,self.filepath_options_frame]
        self.labels = [self.lenght_label,self.velocity_label,self.threshold_label,self.filepath_label]
        self.entries = [self.lenght_entry,self.velocity_entry,self.threshold_entry]
        # self.buttons = [self.entry5_button,self.entry6_button,self.entry7_button,self.entry11_button,self.entry12_button,self.entry8_button]
        self.config()
        self.root.mainloop()
        pass
    ...
    def config(self):
        for frm in self.frames:
            frm.config(bg = GUI_colors.color())
        for labels in self.labels:
            labels.config(bg = GUI_colors.color())
            labels.config(font="helvetica")
        for entry in self.entries:
            entry.config(bg = GUI_colors.color()) 
            entry.config(font="helvetica")

        # for button in self.buttons:
        #     button.config(bg = GUI_colors.color_buttons()) 
    ...
    def preview_voice(self,event=None):
        ...
        if self.preview_status % 2 !=0: #impar
            ...
            # load image
            self.preview_status += 1
            self.panel_frame = Frame(self.filepath_xframe, bd=1, relief="sunken")
    ...
    def preview_combined(self,event=None):
        ...
        plt.imsave('/Users/cmauro/Programs/Xenakis/Update/temp_file/temp_preview.png',summed,cmap="gray")   # save the array to .png 
        # Command to show!
        if self.preview_status == 11:
            self.preview_status = 1
    ...
    def show_output(self):
        fifth_frame = LabelFrame(self.root,text ="Bach.Roll String",padx=3,pady=3,bg=GUI_colors.color()) #pad inside frame
        fifth_frame.grid(row=1,column=0,rowspan=4,columnspan=2,padx=10,pady=10)   #pad outside

        self.entry11_button = Button(fifth_frame, text="Copy to Clipboard",bg="#016F8D",fg="white",relief=RAISED,command=self.clipboard)   #Copy to Clipboard func
        self.entry11_button.grid(row=12,column=0)

        self.entry12_button = Button(fifth_frame, text="Save File",bg="#016F8D",fg="white",relief=RAISED,command=self.save)   #Save File func
        ...
        self.text_output = Text(fifth_frame,width=108,height=20,bg="#22292B",fg="white")
    ...
    def img_proc(self):
        ...
        def primary_proc():
            ...
            return raw_cumulative_out, slice_indexes

        raw_cumulative_out, slice_indexes = primary_proc()

        def mapping_function(raw_cumulative_out):
            ...
            raw_cumulative_out = np.rint(raw_cumulative_out)    # Aproximate
            ...
            return raw_cumulative_out

        raw_cumulative_out = mapping_function(raw_cumulative_out)

        def calc_order_voices(raw_cumulative_out, slice_indexes):
            # Calculate order of voices (From high to low) take average and oder from high to low
            separated_voices = np.split(raw_cumulative_out,slice_indexes)   # Split whole adata into individual voices 
            averages=[]
            cnt = 0
        ...
        for q in out_total_events:
            ...
            # Force break handler 
            if  event_cntr == max_events_per_batch:
                s.append("]")   # False ending

                temp_syntx = "".join(map(str, s))       # Join Everything
                if macro_conter != 0:
                    temp_syntx = f"addchords[{str(counter)}" + temp_syntx

                temp_macro = temp_syntx
                macro_sxyntax.append(temp_macro)
                voice_tracking = counter
                event_cntr = 0  # Reset counter
                temp_macro = [] # Reset temp synt list
                macro_conter += 1
                s = []
            else:
                if sintax[subarray][0,3] == 1:  # Start of voice
                    counter += 1
                ...
                if sintax[subarray][0,3] == -2:              # Commit to .txt if SMALL file
                    s.append("]")
                    temp_syntx = "".join(map(str, s))       # Join Everything
                    macro_sxyntax.append(temp_syntx)
                event_cntr += 1
                subarray += 1                                # go to next sub-array [Event]

        file_out_cntr = 1
        for i in macro_sxyntax: # Dynamic .txt file creator
            print(f"Lenght of batch {len(i)}")
            fname = f"batch_{file_out_cntr}.txt"
